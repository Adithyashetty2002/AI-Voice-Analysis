...
class VoiceAnalysisPipeline:
    ...
    def analyze_audio(self, session_id: str, raw_audio_path: str, topic: str, output_base_dir: str, progress_callback=None) -> dict:
        ...
        try:
            ...
            raw_turns = []
            if os.path.exists(conv_json_beeped_path):
                if progress_callback:
                    progress_callback("Stage 1 & 2: Found cached scrubbed conversation. Skipping Diarization and Transcription...", 75)
                with open(conv_json_beeped_path, "r", encoding="utf-8") as f:
                    raw_turns = json.load(f)
            if not raw_turns:
                diarized_segments = []
                if os.path.exists(diarization_json_path):
                    if progress_callback:
                        progress_callback("Stage 1: Found cached diarization. Skipping Pyannote...", 35)
            ...
            for turn in raw_turns:
                ...
                if spk not in speaker_chunks:
                    speaker_chunks[spk] = []
                speaker_chunks[spk].append(turn)
            if os.path.exists(beeped_wav_path):
                ...
                for spk, chunks in speaker_chunks.items():
                    ...
                    for idx, chunk in enumerate(chunks):
                        start_ms = int(chunk["turn_start"] * 1000)
                        end_ms = int(chunk["turn_end"] * 1000)
